...
non-trivial, but the idea of congruence-closure algorithms is simple.  The algorithm
builds a 
partition of all relevant terms, according to the equalities, and checks for
...
arithmetic on reals, and decision procedures for this theory are often based on
...
to be in contradiction with the bounds on its variables.  If then suffices to
...
combination of those inequalities that reduces to $1 \leq 0$).
...
basically remove from the set of real-feasible solutions a subset with no integer
...
formula.  Such book-keeping is tedious and error-prone.  One strategy is to
...
minimum amount of book-keeping is done in the SAT solver and no book-keeping at
all is done in the theory solvers.\footnote{This is not quite true for the
  ...
  of its machinery.  The same minimal book-keeping is done in this internal SAT
  solver as is done in the main SAT solver.}  If the user asks for a proof, the SAT solver
book-keeping is leveraged to produce the resolution part of the proof.  Proofs of the
...
As mentioned above, the proof format used by CVC4 is LFSC.
Figure~\ref{fig:lfsc} shows an LFSC proof for the example from Section~\ref{sec:intro}.

%TODO: add figure, explain

At the time this article is being written, CVC4 supports proofs for
uninterpreted functions and arrays.  Proofs for the theory of bit-vectors is a
current project and proofs for arithmetic is expected to follow soon thereafter.
